...
-- Output settings
cmd:option('-max_images', -1, 'max number of images to process')
...
local num_process = math.max(#image_paths, opt.max_images)
local results_json = {}
...
if #results_json > 0 then
  ...
  utils.write_json(paths.concat(opt.output_json_dir,'results.json'),out)
end
